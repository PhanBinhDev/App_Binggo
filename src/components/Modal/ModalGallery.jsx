...
const ModalGallery = () => {
  const { onClose, type, isOpen, updateData } = useModal();
  const [isLoading, setIsLoading] = useState(true);
  const [isLoadingFetch, setIsLoadingFetch] = useState(false);
  const [selected, setSelected] = useState("");
  const isModalOpen = isOpen && type === ModalTypes.gallery;
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      className="rounded-[12px] min-w-[400px] min-h-[540px]">
      <ModalContent>
        {(onClose) => (
          <>
            ...
            <ModalBody className="py-3">
              <div className="h-[50px] w-full flex items-center rounded-md bg-slate-100">
              ...
              <div className="flex-1 p-3 rounded-lg bg-slate-100 flex-col flex">
                <div className="flex-1 w-full ">
                  <div className="w-full h-full p-2 overflow-hidden">
                    <div className="grid grid-cols-3 gap-4 max-h-[280px] overflow-y-auto">
                      {isLoading ? (
                        <>
                          {Array.from([1, 2, 3, 4, 5, 6]).map((item, index) => {
                            return (
                              <div
                                key={index}
                                className=" col-span-1 flex items-center justify-center size-[136px] bg-gray-300 rounded animate-pulse text-zinc-600">
                                <ImagePhoto size={40} />
                              </div>
                            );
                          })}
                          ...
                          {galleries.map((item) => {
                            ...
                            return (
                              <div
                                ...
                                className={`cursor-pointer relative col-span-1 w-full flex items-center justify-center h-32 bg-slate-200 hover:bg-slate-300 transition-background rounded-lg border-3 border-solid p-1.5 ${
                                  sameId
                                    ? "border-blue-500"
                                    : "border-transparent"
                                }`}>
                ...
                {/* {isLoadingFetch && (
                  <div className="h-[40px] mt-auto flex items-center justify-center">
